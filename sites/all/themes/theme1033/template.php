...
/**
 * @file
 * Contains theme override functions and process & preprocess functions for theme1033
 */

// Auto-rebuild the theme registry during theme development.
if ( theme_get_setting( 'theme1033_clear_registry' ) ) {
	// Rebuild .info data.
 ...
 */
function theme1033_html_head_alter( &$head_elements ) {
 ...
 */
function theme1033_preprocess_search_block_form( &$vars ) {
 ...
 */
function theme1033_preprocess( &$vars, $hook ) {
	$vars['theme1033_path'] = base_path() . path_to_theme();
 ...
 */
function theme1033_preprocess_html( &$vars ) {
	$vars['doctype'] = _theme1033_doctype();
	$vars['rdf'] = _theme1033_rdf( $vars );
	...
	if ( !$vars['is_front'] ) {
		...
		if ( isset( $page_name ) ) {
			$vars['classes_array'][] = theme1033_id_safe( 'page-' . $page_name );
		}

		$vars['classes_array'][] = theme1033_id_safe( 'section-' . $section );
	...
	if ( ( $d['destination'] == 'blog' ) && ( theme_get_setting( 'theme1033_blog_title' ) != "" ) ) {
		$vars['title'] = theme_get_setting( 'theme1033_blog_title' );
		$vars['head_title'] = theme_get_setting( 'theme1033_blog_title' );
	}
}
 ...
 */
function theme1033_preprocess_page( &$vars ) {
 ...
 */
function theme1033_preprocess_maintenance_page( &$vars ) {
	...
	drupal_add_css( drupal_get_path( 'theme', 'theme1033' ) . '/css/maintenance-page.css' );
}
 ...
 */
function theme1033_preprocess_node( &$vars ) {
	// Striping class
 ...
 */
function theme1033_preprocess_block( &$vars, $hook ) {
 ...
 */
function theme1033_menu_tree( $vars ) {
 ...
 */
function theme1033_field__taxonomy_term_reference( $vars ) {
	$output = '';
 ...
 */
function theme1033_breadcrumb( $vars ) {
	$breadcrumb = isset( $vars['breadcrumb'] ) ? $vars['breadcrumb'] : array();

	$condition = theme_get_setting( 'theme1033_breadcrumb_hideonlyfront' ) ? count( $breadcrumb ) > 1
			: !empty( $breadcrumb );
	$separator = theme_get_setting( 'theme1033_breadcrumb_separator' );

	if ( theme_get_setting( 'theme1033_breadcrumb_showtitle' ) ) {
 ...
 * @return bool
 */
function theme1033_tabs_float() {
	$float = (bool) theme_get_setting( 'theme1033_tabs_float' );
	$float_node = (bool) theme_get_setting( 'theme1033_tabs_node' );
...
function theme1033_id_safe( $string ) {
	// Strip accents
 ...
 * @ingroup themeable
 */
function theme1033_menu_link( array $vars ) {
	...
	$element['#attributes']['class'][] = theme1033_id_safe( $element['#title'] );
 ...
 */
function theme1033_preprocess_menu_local_task( &$vars ) {
 ...
 */
function theme1033_menu_local_tasks( &$vars ) {
	$output = '';
 ...
 */
function theme1033_preprocess_comment( &$variables ) {
...
function theme1033_region_preffix ( $reg ) {
	$block_bg_type = theme_get_setting( 'theme1033_' . $reg . '_bg_type' );
	$block_bg_img = theme_get_setting( 'theme1033_' . $reg . '_bg_img' );
	$block_bg_parallax = theme_get_setting( 'theme1033_' . $reg . '_bg_parallax' );
	$bg_video = theme_get_setting( 'theme1033_' . $reg . '_bg_video' );
	$bg_video_start = theme_get_setting( 'theme1033_' . $reg . '_bg_video_start' );
	$fullwidth = theme_get_setting( 'theme1033_' . $reg . '_fullwidth' );
	...
	if ( isset( $block_bg_img['fid'] ) && $block_bg_img['fid'] && ( $block_bg_type == 'image' ) ) {
		echo 'theme1033_' . $reg . '_bg_img'; echo 'jesse';
...
function theme1033_region_suffix ( $reg ) {
	$fullwidth = theme_get_setting( 'theme1033_' . $reg . '_fullwidth' );
	$block_bg_parallax = theme_get_setting( 'theme1033_' . $reg . '_bg_parallax' );
	$block_bg_img = theme_get_setting( 'theme1033_' . $reg . '_bg_img' );
	...
	if ( $block_bg_parallax ) {
		echo 'theme1033_' . $reg . '_bg_parallax' . ' andy';
		$file = file_load( $block_bg_img['fid'] );
		$url = file_create_url( $file->uri );
...
function theme1033_form_contact_site_form_alter( &$form, &$form_state, $form_id ) {
...
function theme1033_form_comment_form_alter( &$form, &$form_state, $form_id ) {
	...
	$form['comment_body']['und'][0]['#attributes']['placeholder'] = t( "Comment:*" );
	$form['comment_body']['und'][0]['#title_display'] = 'invisible';
}

function theme1033_preprocess_pager(&$vars, $hook) {
	if ($vars['quantity'] > 3) {
		$vars['quantity'] = 3;
	}
	$vars['tags'][0] = 'First';
